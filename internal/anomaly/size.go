		...
		return []Anomaly{
			{
				...
				Deviation:   &dev,
				JobRunID:    &runID,
				Summary:     fmt.Sprintf("backup shrank to %.0f bytes (%.0f%% of expected %.0f)", observed, growthFactor*100, median),
				Details:     details,
			},
		}, nil
	}
	...
	return []Anomaly{
		{
			...
			Deviation:   &dev,
			JobRunID:    &runID,
			Summary:     fmt.Sprintf("backup size anomaly: %.0f bytes (%.1fx median)", observed, growthFactor),
			Details:     details,
		},
	}, nil
}
