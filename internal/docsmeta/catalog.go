...
var FieldDocs = map[string]string{ // #nosec G101 -- values are human-readable docs for config fields; keys like "SFTPConfig.Password"/"S3Config.SecretKey" are field names surfaced in the manual, not credentials.
	...
	"Job.AutoIncludeContainers": "When enabled, newly discovered Docker containers are automatically added to this job on each backup run.",
	...
	"StorageDestination.StageBesideDestination": "Whether classic backups targeting this local destination stage their temporary archives beside it.",
