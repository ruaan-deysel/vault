...
- **Scheduled restore-point verification per job.** Two new columns on `jobs`: `verify_schedule` (cron expression, empty = disabled) and `verify_mode` ("quick" | "deep", default "quick"). The scheduler registers a separate cron entry per job so verification can run on its own cadence independent of the backup schedule (e.g. nightly backups, weekly deep verify). When the cron fires, the runner picks the job's most recent restore point and dispatches a verify run via the existing Feature 3 pipeline (writes to `verify_runs`, broadcasts `verify_progress` / `verify_complete`). Jobs with no restore points yet are silently skipped with a log entry. The Jobs wizard's Advanced step gains a "Scheduled verification" disclosure with a cron text field and a mode dropdown. Verified end-to-end on Unraid with `verify_schedule: "@every 5s", verify_mode: "quick"` — the runner correctly logged repeated "no restore points yet, skipping" entries on each tick.
- **Daily storage destination health check.** The scheduler now runs `TestConnection` against every configured storage destination at 03:30 local (deliberately after typical backup windows so a failing destination is caught before the next morning's backup). Results persist on three new columns on `storage_destinations` — `last_health_check_at`, `last_health_check_status` ("ok" / "failed"), `last_health_check_error` — and broadcast over WebSocket as a `storage_health` event. Each check is capped at 30 s via a goroutine + timeout so a hung adapter never stalls the sweep. The Storage page renders a green "Healthy" / red "Unhealthy" badge per destination with the error message in the tooltip. A new endpoint `POST /api/v1/storage/{id}/health-check` runs an on-demand check and returns `{status, error}`; verified end-to-end through the ok→failed→ok transitions of a local destination.
- **GFS retention preview in the Jobs wizard.** As users tune the `keep_latest` / `keep_daily` / `keep_weekly` / `keep_monthly` / `keep_yearly` fields on an existing job, the wizard now shows "Would keep X of Y current restore points · Z would be pruned · …" 300 ms after the last keystroke. Backed by a new endpoint `GET /api/v1/jobs/{id}/retention-preview?keep_*=N` which runs the GFS classifier against the job's actual restore-point set without applying anything. Returns four arrays (`kept_directly`, `kept_with_ancestors`, `would_delete`, plus `total_restore_points` and `policy_active`). For brand-new jobs (no restore points yet) the preview is hidden and the form shows "Save the job first to see how many existing restore points this policy would keep". Verified end-to-end with seven fabricated restore points across three months: policy `keep_daily=3 keep_weekly=2 keep_monthly=2` correctly identified 5 kept / 2 pruned.
- **Automatic compression downgrade for media-heavy backups.** Folder, plugin, and container-volume backups now pre-scan the source directory once and, if ≥ 70 % of the bytes have already-compressed file extensions (`.mp4`, `.mkv`, `.flac`, `.jpg`, `.zip`, `.gz`, `.zst`, etc. — full list in `internal/engine/compress_auto.go`), automatically downgrade the requested `gzip` / `zstd` compression to `none` for that archive. A backup of a 200 GB Immich library or a Jellyfin media folder now skips the wasted CPU cycle on zstd-compressing already-compressed bytes (savings on those folders is typically < 1 % and not worth the wall time). Logs the decision: `engine: auto-downgrading compression for /path from zstd to none (87% of N bytes is already compressed)`. The runner's manifest records the actually-used compression so restore continues to auto-detect via magic bytes.
- **Restore-point verification (`vault verify`) with quick and deep modes.** New endpoint `POST /api/v1/jobs/{id}/restore-points/{rpid}/verify {"mode":"quick"|"deep"}` returns 202 + a `verify_run_id`; progress is streamed over the existing WebSocket as `verify_started` / `verify_progress` / `verify_complete` events and persisted in a new `verify_runs` table (id, restore_point_id, mode, status, files_checked, files_failed, bytes_read, started_at, completed_at, error_summary). **Quick mode** runs `adapter.Stat()` on every recorded file in the restore point's `metadata.checksums` map and fails on missing files or size mismatches — no data transfer, suitable for weekly scheduled runs. **Deep mode** streams every file and recomputes SHA-256 against the recorded checksum from the backup-time manifest — catches silent bit rot on cold storage. Two helper endpoints round out the API: `GET /api/v1/jobs/{id}/verify-runs/{vrid}` returns one run's state and `GET /api/v1/jobs/{id}/restore-points/{rpid}/verify-runs?limit=10` returns recent verifications for a restore point. Verified end-to-end against Unraid: 3-file folder backup → quick verify passed in <1 s, deep verify passed reading 477 B, single-file corruption → deep verify failed with `files_failed=1` and exact error message naming the bad file. v1 scope: manual + API-triggered verification. Scheduled verification (weekly quick / monthly deep) is a follow-up. The runner mutex is not taken — verify runs in its own goroutine and does not block scheduled backups.
- **Partial restore via tar index sidecar.** Folder, plugin, and container-volume backups now write a small JSON sidecar (`<archive>.index.json`) next to each tar archive listing every entry's path, size, mode, and mtime. The sidecar is auto-encrypted alongside the archive when job-level encryption is enabled. A new API endpoint `GET /api/v1/jobs/{id}/restore-points/{rpid}/contents?item=<name>[&file=<archive>]` returns the parsed index (decrypting on the fly when needed). The restore endpoint `POST /api/v1/jobs/{id}/restore` now accepts an optional `file_paths: {itemName: ["relative/path", ...]}` map; supplied paths cause the folder/plugin engine handlers to extract only those entries (descendants of selected directories are included automatically). Backups produced before this feature shipped continue to restore everything (the indexer is best-effort and a missing sidecar falls back to the legacy whole-archive extract path). VM, ZFS, and container-image archives are unaffected. **v1 ships the backend + API + selective-extract pipeline; the restore-wizard file picker UI follows in a separate change.** Verified end-to-end against the cloud-storage smoke (S3 + WebDAV) plus a live folder backup that selectively restored 2 of 3 source files.
- **Grandfather-father-son (GFS) retention policy on jobs.** New optional fields `keep_latest`, `keep_daily`, `keep_weekly`, `keep_monthly`, `keep_yearly` on each job (all default `0` = disabled). When any of the five is > 0 the runner switches to GFS classification: it walks restore points newest-first and keeps each as long as it's one of the first N latest, OR the first-seen point in its day/week/month/year bucket and that bucket still has slots. A single restore point can satisfy multiple buckets at once (yesterday's only backup fills today's latest AND today's daily AND this week's weekly slot). Existing `retention_count` / `retention_days` continue to work unchanged on every other job — GFS is purely additive and opt-in. Chain-ancestor protection (`chain_health.go`) still applies on top so kept incrementals/differentials always have a restorable parent. The Jobs wizard adds a "Long-term retention (GFS)" disclosure under the existing Retention Policy panel with a callout when GFS is active.

### Fixed

- **S3-compatible uploads to MEGA, Backblaze B2, IDrive E2, and similar gateways should no longer fail with `SignatureDoesNotMatch: Request signature does not match` on every PutObject while Test Connection succeeds (closes #88 follow-up).** Since `aws-sdk-go-v2` v1.32, every `PutObject` / `UploadPart` is signed with an `x-amz-checksum-crc32` trailer that AWS S3 includes in its SigV4 canonicalization. Most S3-compatible gateways do not consume that trailer, so they recompute the signature without it and respond `403 SignatureDoesNotMatch`. `HeadBucket` (Test Connection) has no body, no trailer, and succeeds — producing the characteristic "test passes, upload always fails" fingerprint reported on the MEGA S3 endpoint and previously on Backblaze B2. The S3 adapter now sets `RequestChecksumCalculation` and `ResponseChecksumValidation` to `WhenRequired` automatically whenever a custom `endpoint` is configured (i.e. user is not talking to canonical AWS S3), so the trailer is omitted and the gateway accepts the request. Real AWS (blank Endpoint) keeps the SDK default (`WhenSupported`) so checksums continue to provide end-to-end integrity there. Vault's own SHA-256 verification in the runner (`verify_backup`) continues to guarantee object integrity end-to-end either way.
- **S3 / Backblaze B2 uploads of multi-TB objects should no longer abort with `exceeded total allowed S3 limit MaxUploadParts (10000)` (closes #95 follow-up from @celborn).** After the previous fix raised the upload timeout in 2026.05.02, a fresh failure surfaced when celborn's ~281 GB Immich-Backblaze job ran to completion: the AWS SDK v2 `transfermanager` used its default `PartSizeBytes = 8 MiB`, which combined with the S3 protocol's hard 10,000-part-per-object cap to enforce an 80 GB ceiling. The SDK has built-in auto-scaling for known-size streams, but Vault's age-encrypted uploads pass an `io.PipeReader` (`crypto.EncryptReader`) whose size is unknown up-front, so the auto-scaler at `transfermanager/api_op_UploadObject.go:864` never triggered. The S3 adapter now binds `PartSizeBytes` to the uploader at construction time, defaulting to 64 MiB (640 GB ceiling — comfortable for whole-folder home backups) and exposing a new optional `part_size_mb` field on the S3 storage config so power users can raise it for multi-TB datasets (256 → 2.5 TB, 1024 → 10 TB). Validation rejects values outside the S3 spec range of 5 MiB – 5 GiB. The Storage form gains a "Part size (MiB)" input under the existing Advanced section alongside Upload timeout.
- **WebDAV chunk uploads now use exponential backoff with HTTP-status-aware retry classification.** The previous per-chunk retry schedule was a fixed two-step `[100ms, 500ms]` slice that did not distinguish transient transport failures from permanent client errors. Chunk PUTs now retry on an exponential schedule (`100ms → 300ms → 1s → 3s → 8s`, 6 attempts) **only** for retriable conditions: HTTP `423 Locked`, `409 Conflict`, `429 Too Many Requests`, any `5xx`, the stall watchdog (`ErrUploadStalled`), and non-HTTP transport errors (dial, TLS, EOF). Permanent `4xx` failures (`401`, `403`, `404`, `413`, …) fail fast on the first attempt with a clear log line, so misconfigured credentials or quota-exceeded responses surface immediately instead of consuming the full retry budget. HTTP status codes are extracted from `gowebdav`'s `*os.PathError` wrapping via `strconv.Atoi(strings.Split(pe.Err.Error(), " ")[0])`.
- **WebDAV uploads should no longer buffer non-seekable backup streams into memory or leave encryption goroutines alive after stalled retries (closes #96).** The age encryption wrapper now returns an `io.ReadCloser`, the runner closes it on every upload attempt exit path, and the WebDAV stall wrapper closes its wrapped reader instead of only stopping the watchdog. WebDAV writes now use `gowebdav.WriteStreamWithLength` for bounded objects so the `gowebdav.WriteStream` fallback that copies unknown-length streams into RAM is avoided.
- **Large WebDAV backups are now split into manifest-backed chunks instead of one monolithic PUT (closes #97).** Files larger than the configured WebDAV chunk size are uploaded as independent fixed-size chunk objects with per-chunk SHA-256 hashes and a manifest written last. Restore, verify, list, stat, and delete are manifest-aware while old single-file WebDAV backups remain readable.

### Added

- **Configurable WebDAV chunk size in the Storage form and WebDAV config JSON.** The default `chunk_size_mb: 0` uses 50 MiB chunks, positive values set the chunk size in MiB, and negative values disable chunking while still using memory-safe known-length writes.
- **Configurable S3 multipart part size (`part_size_mb`) in the Storage form and S3 config JSON.** Default `0` uses 64 MiB parts (640 GB per-object ceiling — fits typical home-server workloads). Raise to 256 (2.5 TB), 512 (5 TB), or 1024 (10 TB) for very large datasets such as full Immich libraries or whole-disk images. Range is enforced at 5–5120 MiB per the S3 protocol. Peak upload memory ≈ `part_size × concurrency` (default 5), so 1 GiB parts cost roughly 5 GiB of RAM during an active upload.
