...
- **WebDAV uploads no longer buffer non-seekable backup streams into memory or leave encryption goroutines alive after stalled retries (closes #96).** The age encryption wrapper now returns an `io.ReadCloser`, the runner closes it on every upload attempt exit path, and the WebDAV stall wrapper closes its wrapped reader instead of only stopping the watchdog. WebDAV writes now use `gowebdav.WriteStreamWithLength` for bounded objects so the `gowebdav.WriteStream` fallback that copies unknown-length streams into RAM is avoided.
