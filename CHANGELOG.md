...
- **WebDAV chunk uploads now use Kopia-style exponential backoff with HTTP-status-aware retry classification.** The previous per-chunk retry schedule was a fixed two-step `[100ms, 500ms]` slice that did not distinguish transient transport failures from permanent client errors. Inspired by `kopia/repo/blob/webdav/webdav_storage.go` and `kopia/internal/retry`, chunk PUTs now retry on an exponential schedule (`100ms → 300ms → 1s → 3s → 8s`, 6 attempts) **only** for retriable conditions: HTTP `423 Locked`, `409 Conflict`, `429 Too Many Requests`, any `5xx`, the stall watchdog (`ErrUploadStalled`), and non-HTTP transport errors (dial, TLS, EOF). Permanent `4xx` failures (`401`, `403`, `404`, `413`, …) fail fast on the first attempt with a clear log line, so misconfigured credentials or quota-exceeded responses surface immediately instead of consuming the full retry budget. HTTP status codes are extracted from `gowebdav`'s `*os.PathError` wrapping using the same parser Kopia uses (`strconv.Atoi(strings.Split(pe.Err.Error(), " ")[0])`).
